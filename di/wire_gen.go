...
func InitializeApp() (*server.App, error) {
	...
	walletUsecase := usecase.NewWalletUsecase(walletRepo, txManagerItf, env)
	...
	auctionCacheRepo := repository.NewAuctionCacheRepo(client)
	bidRepo := repository.NewBidRepo(db)
	seaweedFSStorage := config.NewSeaweedFSStorage(env)
	auctionUsecase := usecase.NewAuctionUsecase(auctionRepo, auctionCacheRepo, walletRepo, bidRepo, userRepo, seaweedFSStorage, txManagerItf, client)
